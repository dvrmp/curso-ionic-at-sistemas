...
export class FruitsListPage implements OnInit {
  ...
  public fruits: Fruit[];
  private fruitsBackup: Fruit[] = [];
  public hasBeenFruitUpdated: Boolean = false;
  ...
  public ngOnInit(): void {
    ...
    this.fruitsBackup = this.fruits;
  }

  public deleteFruit(index: number): void {

    console.log('TODO: Remove fruit index', index);
    this.fruits = this.fruits.filter((fruit) => fruit.id !== this.fruits[index].id);
    if(this.hasBeenFruitUpdated === false) this.hasBeenFruitUpdated = true;

  }
  public restore(): void {
    this.hasBeenFruitUpdated = false;
    this.fruits = this.fruitsBackup;
  }

  public navigateToSandbox() : void {
    this.navController.navigateRoot('sandbox');
  }
}
